...
class MedicalSearcher:
    ...
    def search(self, query, k=5):
        ...
        for score, idx in zip(scores[0], indices[0]):
            results.append({
                ...
                'text_preview': self.df.iloc[idx][self.text_column][:200]
            })
        return results
    ...
    def save(self, path=None):
        ...
        with open(f"{path}/meta.txt", "w") as f:
            f.write(self.text_column or "cleaned_transcription")
    ...
    def load(self, path=None):
        ...
        try:
            with open(f"{path}/meta.txt") as f:
                self.text_column = f.read().strip()
        except FileNotFoundError:
            self.text_column = 'cleaned_transcription'
        print(f"Index loaded: {self.index.ntotal} documents")


def evaluate_search(searcher, df, k_values=(1, 3, 5), n_queries=100, text_column=None):
    text_column = text_column or searcher.text_column or 'cleaned_transcription'
    sample = df.sample(n=n_queries, random_state=42)
    results = {}
    for k in k_values:
        precisions = []
        for _, row in sample.iterrows():
            p = searcher.precision_at_k(row[text_column][:500], row['medical_specialty'], k=k)
            precisions.append(p)
